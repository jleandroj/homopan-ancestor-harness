...
**ENFORCEMENT:** a PreToolUse hook (`.claude/gate_check.sh`, registered in `.claude/settings.json`) **BLOCKS** `Write/Edit/NotebookEdit/Bash` unless `init.sh` has generated a valid gate pass. The gate hashes the full contract surface (CLAUDE.md, agents.md, gate_check.sh, bitacora_log.sh, settings.json, init.sh) **plus the `skills/` tree and the boundary scripts** (`scripts/sandbox_run.sh`, `scripts/net_wrappers/`, `egress_allowlist.txt`) -- content-based, not time-based.
