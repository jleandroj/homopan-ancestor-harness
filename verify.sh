...
for t in tests/test_idempotency.sh tests/test_jobstore_guard.sh tests/test_sandbox.sh tests/test_e2e_synthetic.sh tests/test_e2e_mock.sh tests/test_gate.sh tests/test_gate_sandbox.sh; do
  if [[ -f "${t}" ]]; then
